...
class MandallaCue extends Cue {
  ...
  void setup() {
    ...
    img = loadImage("mandalla/image.jpg");
    ...
    for (int i = 0; i < snakes.length; i++) {
      ...
      snakes[i] = new Snake(dividemidPoints[i], int(random(360)), snakeWidthfactor, int(random(1,7)), img);
    }
  }
...
class Snake {
  ...
  Snake(float radius, int degrees, float step, int randnum, PImage iimg) {
    img = iimg;
    r = radius;
    deg = degrees;
    begindeg = degrees;
